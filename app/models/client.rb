class Client < ApplicationRecord
	...
	def self.save_client_logo_file(file_name,display_name,client_name,image_path)
		doc_obj=Client.new
		doc_obj.file_name = file_name
		doc_obj.display_name = display_name
		doc_obj.client_name = client_name
		doc_obj.image_path = image_path
		doc_obj.save
	end
	...
	def self.get_client_images
		...
		# return Client.find_by_sql("select * from clients order by display_name").collect{|x| x.image_path}
	end
end
